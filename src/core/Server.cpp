...
void Server::start(const std::vector<ServerConfig>& configs) {
    ...
    for (size_t i = 0; i < configs.size(); ++i) {
        ...
        std::cout << "[" << "\033[33m"<< "INFO" << RESET_COLOR <<"] Binding to " << configs[i].host << ":" << configs[i].port << std::endl;
        ...
        if (getsockname(serverSocket, (struct sockaddr*)&boundAddr, &boundLen) == 0) {
            ...
            std::cout << "[" << INFO_COLOR << "INFO" << RESET_COLOR <<"] Socket created successfully." << std::endl;
            std::cout << "[" << INFO_COLOR << "INFO" << RESET_COLOR <<"] Socket bound to : " << "\033[33m" << boundIP << ":" << ntohs(boundAddr.sin_port) << RESET_COLOR << std::endl;
        }
